...
fact{ //Two-way relation
    all o:Farmer, f:Farm | 
        f in o.ownedFarm <=> f.farmOwner = o
}

fact{ //Two-way relation
    all ag:Agronomist, ar:Area | 
        ag in ar.areaAgronomists <=> ag.agronomistArea = ar 
}

fact{ //Two-way relation
    all f:Farm, ar:Area |
        f in ar.areaFarms <=> f.farmArea = ar
}
...
fact {
    ...
    all r:Report, v:Visit | v in r.reportVisit <=> v.visitCompleted = True
}
